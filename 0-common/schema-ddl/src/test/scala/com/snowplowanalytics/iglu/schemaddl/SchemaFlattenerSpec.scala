...
class SchemaFlattenerSpec extends Specification {
  def is = s2"""
  Check FlatSchema
  ...
  def e1 = {
    val property = Map(
      "test_key" -> Map("type" -> "string,null,integer", "maxLength" -> "32", "maximum" -> "1024"))
    val result = Map(
  ...
  def e2 = {
    val jValues =
      List(JInt(3), JNull, JString("str"), JDecimal(3.3), JDouble(3.13), JString("another_str"))
  ...
  def e3 = {
    val jValues = List(
      ...
      JDouble(3.13),
      JString("another_str"),
      JObject(List(("keyOfFatum", JInt(42)))))
    FlatSchema.stringifyArray(jValues).toEither must beLeft
  }

  def e4 = {
    implicit val formats = DefaultFormats
    val json: JObject = parse(
      Source.fromURL(getClass.getResource("/schema_with_required_properties.json")).mkString)
      .asInstanceOf[JObject]
    val map = json.extract[Map[String, JValue]]

    FlatSchema.getRequiredProperties(map).toEither must beRight(
      List("anotherRequired", "requiredKey"))
  ...
  def e6 = {
    ...
    FlatSchema.processProperties(root, requiredAccum = Set("root")).toEither must beRight(
      FlatSchema.SubSchema(resultMap, Set("root.requiredKey", "root.anotherRequired")))
  }
  ...
  def e7 = {
    ...
    FlatSchema.flattenJsonSchema(schema, splitProduct = true).toEither must beRight.like {
      case flatSchema =>
        flatSchema must beEqualTo(FlatSchema(ListMap.empty[String, Map[String, String]]))
    }
  }
  ...
  def e8 = {
    ...
    FlatSchema.flattenJsonSchema(json, splitProduct = true).toEither must beRight.like {
      case flatSchema =>
        flatSchema must beEqualTo(
          FlatSchema(ListMap("nested.object_without_properties" -> Map("type" -> "string"))))
    }
  }
  ...
  def e10 = {
    ...
    FlatSchema.flattenJsonSchema(schema, splitProduct = false) mustNotEqual Success(
      FlatSchema(result, Set.empty))
  }.orSkip
}
