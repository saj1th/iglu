...
object Suggestion {
  ...
  val timestampSuggestion: Suggestion = (schema, required) =>
    (schema.`type`, schema.format) match {
      ...
      case (Some(CommonProperties.Product(types)), Some(StringProperties.DateFormat))
          if withNull(types, CommonProperties.String) =>
      ...
      case (Some(CommonProperties.Product(types)), Some(StringProperties.DateTimeFormat))
          if withNull(types, CommonProperties.String) =>
  ...
  def finalSuggestion(schema: Schema, required: Boolean): String => Field =
    schema.`type` match {
      ...
      case _ =>
        name =>
          Field(name, Type.String, Mode.required(required))
    }
  ...
  private[iglu] def fromEnum(enums: List[JValue], required: Boolean): String => Field = {
    ...
    def isNumeric(json: JValue) =
      json.isInstanceOf[JInt] || json.isInstanceOf[JDouble] || json
        .isInstanceOf[JDecimal] || json == JNull
    val noNull: Boolean = !enums.contains(JNull)

    if (enums.forall(isString)) { name =>
      Field(name, Type.String, Mode.required(required && noNull))
    } else if (enums.forall(isInteger)) { name =>
      Field(name, Type.Integer, Mode.required(required && noNull))
    } else if (enums.forall(isNumeric)) { name =>
      Field(name, Type.Float, Mode.required(required && noNull))
    } else { name =>
      Field(name, Type.String, Mode.required(required && noNull))
    }
  }
  ...
  private def onlyNumeric(types: Set[CommonProperties.Type], allowNull: Boolean): Boolean =
    if (allowNull)
      types == Set(CommonProperties.Number, CommonProperties.Integer, CommonProperties.Null)
